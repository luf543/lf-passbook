...
var TOP_LEVEL           = [ "authenticationToken", "backgroundColor", "barcode", "description",
    ...
    "suppressStripShine", "webServiceURL", "appLaunchURL", "associatedStoreIdentifiers", "expirationDate"];
...
Pass.prototype.pipe = function(output) {
    ...
    function doneWithImages() {
        if (lastError) {
            zip.close();
            self.emit("error", "zipClose:"+lastError);
        } else {
            self.emit("info", "processStart");
            process.nextTick(function() {
...
Pass.prototype.render = function(response, callback) {
    // response.setHeader('Content-disposition', 'attachment; filename='+downname+'.pkpass');
